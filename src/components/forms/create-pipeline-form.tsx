...
const CreatePipelineForm: React.FC<CreatePipelineFormProps> = ({
  ...
  const onSubmit = async (values: z.infer<typeof CreatePipelineFormSchema>) => {
    ...
    try {
      ...
      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `Mise à jour d'un pipeline | ${response?.name}`,
        subaccountId: subAccountId,
      })

      toast({
        title: 'Succès',
        description: 'Détails du pipeline enregistrés',
      })
      router.refresh()
    } catch (error) {
      toast({
        variant: 'destructive',
        title: 'Oups!',
        description: 'Impossible d\'enregistrer les détails du pipeline',
      })
    }
  ...
  return (
    <Card className="w-full ">
      <CardHeader>
        <CardTitle>Détails du pipeline</CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Nom du pipeline</FormLabel>
                  <FormControl>
                    <Input
                      placeholder="Nom"
                      {...field}
